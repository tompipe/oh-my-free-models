...
| 命令 | 用途 |
| --- | --- |
| `omfm start` | 在前台启动代理，并输出 request/response 路由日志。用 `Ctrl+C` 停止。 |
...
前台 `omfm start` 还会输出单行请求/响应日志；可用时包含 requested model、routed model、route reason、cached latency、status、duration 和 stream 标记。
...
- `POST /anthropic/messages/count_tokens`（别名）

`omfm` 接收本地 Anthropic 认证头，再用匹配的 provider 密钥转发请求。如果 provider 自身暴露了 Anthropic 兼容端点（如 OpenRouter 的 Anthropic surface），`omfm` 会直接使用；否则 fallback 到 Anthropic/OpenAI 翻译层，支持文本和常见客户端 tool-use 流程。Token count 返回本地兼容性估算值，不是 provider tokenizer 的精确计数。
...
- `omfm start` 在代理运行期间也会在后台刷新已选模型的 latency。
